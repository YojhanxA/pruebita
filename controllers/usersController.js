...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({ ok: false, error: { message: "Usuario no encontrado o credenciales incorrectas" } });
        }

        const token = jwt.sign({ userId: user.id }, process.env.SECRET_JWT, { expiresIn: "1h" });
        res.status(200).json({ ok: true, msg: "Usuario autenticado correctamente", user, token });
    } catch (error) {
        console.error("Error al autenticar usuario:", error.message);
        res.status(500).json({ ok: false, error: { message: "Error en el servidor" } });
    }
};

const register = async (req, res) => {
    try {
        const { nombre, edad, genero, preferencias, ubicacion, fotoPerfil, email, password } = req.body;

        const existingUser = await Users.findOne({ email });
        if (existingUser) {
            return res.status(409).json({ message: "El usuario ya existe con este correo electrónico" });
        }
...
const swipe = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: "Error en el servidor al procesar el swipe" });
    }
};
...
const getMatches = async (req, res) => {
    ...
    try {
        ...
        }).populate('usuario1_id', 'nombre fotoPerfil').populate('usuario2_id', 'nombre fotoPerfil');

        // Formatear la respuesta para que sea más fácil de usar en el frontend
        const formattedMatches = matches.map(match => {
            const matchedUser = match.usuario1_id.id === userId ? match.usuario2_id : match.usuario1_id;
            return {
                id: match.id,
                user: matchedUser,
        ...
        res.status(500).json({ message: "Error en el servidor al obtener los matches" });
    }
};
...
module.exports = {
    ...
    swipe,
    getMatches,
    authenticateToken // Exporta el middleware de autenticación
};